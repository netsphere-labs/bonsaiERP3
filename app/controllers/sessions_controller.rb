# encoding: utf-8
# author: Boris Barroso
# email: boriscyber@gmail.com
class SessionsController < ApplicationController
  skip_before_action :set_tenant, :check_authorization!
  before_action :check_logged_in, only: %w(new create)
  layout 'sessions'

  def new
    @session = Session.new
  end

  def create
    @session = Session.new(session_params)

    case
    when @session.authenticate?
      session[:user_id] = @session.user_id
      flash[:notice] = "Ha ingresado correctamente."

if USE_SUBDOMAIN      
      redirect_to home_url(host: UrlTools.domain, subdomain: @session.tenant)
else
      redirect_to home_url()
end
    else
      flash.now[:error] = 'El email o la contraseña que ingreso son incorrectos.'

      render :new
    end
  end

  def destroy
    reset_session

    redirect_to login_url(host: UrlTools.domain, subdomain: 'app'), notice: "Ha salido correctamente."
  end


private

  def check_logged_in
    if session[:user_id] && u = User.active.find(session[:user_id])
      if org = u.organisations.first
if USE_SUBDOMAIN      
        redirect_to home_url(host: UrlTools.domain, subdomain: org.tenant),
                    notice: 'Ha ingresado correctamente.'
else
        redirect_to home_url(), notice: 'Ha ingresado correctamente.'
end
      else
        reset_session
      end
    end
  end

  
    def session_params
      params.require(:session).permit(:email, :password)
    end
end
