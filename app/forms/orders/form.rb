...
class Orders::Form < BaseForm
  ...
  def initialize order
    ...
    super()
    @model_obj = order
    order.details.each do |r| r.edit_price = r.unit_price end
  ...
  def assign model_params, detail_params
    ...
    @details = Orders::Form.create_details_from_params(model_obj, detail_params)
    model_obj.gross_total = @details.inject(BigDecimal(0)) {|r, item|
                              r += item.amount } 
    model_obj.total = model_obj.gross_total
  end
  ...
  def save!
    ...
    OrderDetail.where(order_id: model_obj.id).delete_all
...
private
  ...
  def self.create_details_from_params(order, detail_params)
    #raise detail_params.inspect # DEBUG

    ary = []
    #curr = Money::Currency.find(order.currency)
    detail_params.each do |_lineno, h|
      m = OrderDetail.new h.permit(:item_id, :edit_price, :quantity, :description)
      m.amount = nominal_amount((m.edit_price || 0.0) * (m.quantity || 0.0),
                                order.currency)
      (ary << m) if (m.quantity || 0.0) != 0.0
    end
