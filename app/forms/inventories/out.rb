...
# form object for transfer-in-2-steps - out
class Inventories::Out < Inventories::Form
  ...
  def self.create_details_from_params detail_params, store_id
    ...
    detail_params.each do |_lineno, h|
      ...
      m.movement_type = 303  # Goods issue for transfer ここが異なるので別に定義
      m.store_id = store_id
  ...
  def build_details_from_order
    order.details.each do |det|
      # 転送出庫は, `balance` そのまま (MUST)
      self.details << InventoryDetail.new(item_id: det.item_id, quantity: det.balance)
    end
  end
